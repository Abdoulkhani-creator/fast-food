...
<section class="accounts">
    ...
    <div class="box-container">
        ...
        <?php
        $select_account = mysqli_query($connect, "SELECT * FROM `directeur`");

        if ($select_account && mysqli_num_rows($select_account) > 0) {
            while ($fetch_accounts = mysqli_fetch_assoc($select_account)) {
                ?>
                <div class="box">
                    <p> ID de l'administrateur : <span><?= $fetch_accounts['IdentifiantDirecteur']; ?></span> </p>
                    <p> Nom d'utilisateur : <span><?= $fetch_accounts['Nom']; ?></span> </p>
                    <div class="flex-btn">
                    <a href="admin_supprimer.php?delete=<?= $fetch_accounts['IdentifiantDirecteur']; ?>" class="delete-btn" onclick="return confirm('Supprimer ce Directeur ?');">Supprimer</a>
                        <?php
                        if ($fetch_accounts['Nom'] == $admin_id) {

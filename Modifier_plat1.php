...
if (isset($_POST['add_product'])) {
    ...
    if (move_uploaded_file($_FILES['image']['tmp_name'], $upload_directory . $image)) {
        ...
        $requete_insertion = "UPDATE plat SET Prix='$prix_produit', DescriptionPlat='$commentaire', Categorie ='$categorie', CheminImage='$image_path', NomPlat='$nom_produit' WHERE IdentifiantPlat='$num'";

        // Exécuter la requête
        if (mysqli_query($connect, $requete_insertion)) {
            echo "Le produit a été modifié avec succès.";
        } else {
            echo "Erreur lors de la modification du produit : " . mysqli_error($connect);
        }
    } else {

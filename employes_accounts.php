...
<section class="accounts">
   ...
   <h1 class="heading">Comptes Employés</h1>

   <div class="box-container">

      <div class="box">
         <p>Ajouter un nouvel employé</p>
         <a href="ajouter_employe.php" class="option-btn">Ajouter</a>
      </div>
      <?php
$username = "root";
$servername = "localhost";
...
$select_account = mysqli_query($connect, "SELECT NumeroTelephone as id_employe, Nom, Prenom, Statut as poste FROM employe");

if ($select_account && mysqli_num_rows($select_account) > 0) {
    while ($fetch_accounts = mysqli_fetch_assoc($select_account)) {
        ?>
        <div class="box">
            <p>Numero de telephone du l'employé : <span><?= $fetch_accounts['id_employe']; ?></span></p>
            <p>Nom de l'employé : <span><?= $fetch_accounts['Nom']; ?></span></p>
            <p>Poste de l'employé : <span><?= $fetch_accounts['poste']; ?></span></p>
            <div class="flex-btn">
                <a href="Modifier_employer.php?id=<?= $fetch_accounts['id_employe']; ?>" class="option-btn">Modifier</a>
                <a href="supprimer_employe.php?id=<?= $fetch_accounts['id_employe']; ?>" class="delete-btn" onclick="return confirm('Supprimer ce compte ?');">Supprimer</a>
            </div>
        </div>
        <?php
    }
